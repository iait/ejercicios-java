...
public class LocalidadRepositoryIT {
    ...
    public void givenExistLocalidad_thenUpdated() {
        ...
        final long localidadId = 1L;
        final String localidadNombre = "TEST";
        
        LocalidadRepository localidadRepository = new LocalidadRepository(dataSource);
        localidadRepository.setProvinciaRepository(provinciaRepository);
        
        Optional<LocalidadEntity> optional = localidadRepository.findById(localidadId);
        assertThat(optional).isPresent();
        
        LocalidadEntity entity = optional.get();
        
        entity.setNombre(localidadNombre);
        localidadRepository.save(entity);
        
        assertThat(entity.getNombre()).isEqualTo(localidadNombre);
        
        Optional<LocalidadEntity> saved = localidadRepository.findById(localidadId);
        assertThat(saved).isPresent().contains(entity);
        
        assertThat(saved.get().getNombre()).isEqualTo(localidadNombre);
        
    }
    
    @Test
    @Transactional
    public void eliminaLocalidadExistente() {
        ...
        Optional<LocalidadEntity> before = localidadRepository.findById(localidadId);
        assertThat(before).isPresent();
        
        Optional<LocalidadEntity> deleted = localidadRepository.deleteById(localidadId);
        assertThat(deleted).isPresent();
        assertThat(deleted).isEqualTo(before);
        
        Optional<LocalidadEntity> after = localidadRepository.findById(localidadId);
        assertThat(after).isNotPresent();
    }
    
    @Test
    @Transactional
    public void eliminaLocalidadNoExistente() {
        
        final long localidadId = 10L;
